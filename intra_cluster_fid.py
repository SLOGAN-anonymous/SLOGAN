...
def calculate_cluster_distance(x_real, x_fake, num_class, sess, eps=1e-6):
    ...
    #diff = np.einsum('ik,jk->ijk',mu1,-mu2)#mu1 - mu2
    ...
    for i in range(num_class):
        for j in range(num_class):
            ...
            # product might be almost singular
            covmean, _ = linalg.sqrtm(sigma1[j].dot(sigma2[i]), disp=False)#np.einsum('ijk,lkm->iljm',sigma1[j],sigma2[i])#np.einsum('ijk,lkj->ilj',sigma1,sigma2)#
            if not np.isfinite(covmean).all():
                msg = "fid calculation produces singular product; adding %s to diagonal of cov estimates" % eps
                warnings.warn(msg)
